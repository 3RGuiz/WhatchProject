...
EMOTION_CONFIG <- list(
  "rire" = list(
    ...
    category = "positive",
    # Poids multi-critères optimisés
    weights = list(genre = 0.30, quality = 0.25, popularity = 0.30, recency = 0.10, duration = 0.05),
    min_votes = 1000  # Les comédies ont besoin de validation
  ),
  "detente" = list(
    ...
    category = "positive",
    weights = list(genre = 0.25, quality = 0.30, popularity = 0.30, recency = 0.10, duration = 0.05),
    min_votes = 800
  ),
  "romance" = list(
    ...
    category = "positive",
    weights = list(genre = 0.35, quality = 0.30, popularity = 0.20, recency = 0.10, duration = 0.05),
    min_votes = 500
  ),
  "voyage" = list(
    ...
    alpha_preference = 0.6, rating_min = 7.2, duration_preference = "long",
    category = "positive",
    weights = list(genre = 0.30, quality = 0.35, popularity = 0.20, recency = 0.10, duration = 0.05),
    min_votes = 1500
  ),
  "famille" = list(
    label = "En famille", description = "Films adaptés à tous les âges",
    genres = c("Family", "Animation"),
    alpha_preference = 0.8, rating_min = 7.2, duration_preference = "moyen",
    exclude_genres = c("Horror", "Thriller", "War", "Crime"),
    category = "positive",
    weights = list(genre = 0.30, quality = 0.35, popularity = 0.25, recency = 0.05, duration = 0.05),
    min_votes = 2000  # Films familiaux doivent être validés
  ),
  ...
  "action" = list(
    ...
    alpha_preference = 0.7, rating_min = 6.8, duration_preference = "moyen",
    category = "intense",
    weights = list(genre = 0.35, quality = 0.25, popularity = 0.25, recency = 0.10, duration = 0.05),
    min_votes = 2000,
    recency_boost = 0.15  # L'action vieillit vite
  ),
  "peur" = list(
    label = "Peur", description = "Films d'horreur pour avoir peur",
    genres = c("Horror"), exclude_genres = c("Comedy", "Family", "Animation"),
    alpha_preference = 0.3, rating_min = 6.5, duration_preference = "moyen",
    category = "intense",
    weights = list(genre = 0.40, quality = 0.35, popularity = 0.15, recency = 0.05, duration = 0.05),
    min_votes = 500  # Horreurs de niche acceptées
  ),
  "angoisse" = list(
    ...
    alpha_preference = 0.25, rating_min = 7.2, duration_preference = "moyen",
    category = "intense",
    weights = list(genre = 0.35, quality = 0.40, popularity = 0.15, recency = 0.05, duration = 0.05),
    min_votes = 1000
  ),
  "epique" = list(
    ...
    alpha_preference = 0.5, rating_min = 7.7, duration_preference = "long",
    category = "intense",
    weights = list(genre = 0.30, quality = 0.45, popularity = 0.15, recency = 0.05, duration = 0.05),
    min_votes = 2000
  ),
  ...
  "pleurer" = list(
    ...
    alpha_preference = 0.15, rating_min = 7.7, duration_preference = "long",
    category = "profonde",
    weights = list(genre = 0.25, quality = 0.50, popularity = 0.10, recency = 0.10, duration = 0.05),
    min_votes = 1000
  ),
  "reflechir" = list(
  ...
  "mystere" = list(
    ...
    alpha_preference = 0.35, rating_min = 7.2, duration_preference = "moyen",
    category = "profonde",
    weights = list(genre = 0.35, quality = 0.40, popularity = 0.15, recency = 0.05, duration = 0.05),
    min_votes = 1200
  ),
  "interroger" = list(
    ...
    alpha_preference = 0.25, rating_min = 7.3, duration_preference = "long",
    category = "profonde",
    weights = list(genre = 0.35, quality = 0.45, popularity = 0.12, recency = 0.03, duration = 0.05),
    min_votes = 1500
  ),
  "contempler" = list(
    ...
    alpha_preference = 0.05, rating_min = 7.9, duration_preference = "any",
    category = "profonde",
    weights = list(genre = 0.20, quality = 0.55, popularity = 0.05, recency = 0.15, duration = 0.05),
    min_votes = 500  # Films contemplatifs souvent confidentiels
  ),
  
  "nostalgie" = list(
    label = "Nostalgie", description = "Classiques des années 70-90",
    genres = NULL, alpha_preference = 0.4, rating_min = 7.7, duration_preference = "any",
    year_range = c(1970, 1999), category = "specifique",
    weights = list(genre = 0.15, quality = 0.50, popularity = 0.25, recency = 0.05, duration = 0.05),
    min_votes = 3000,  # Classiques doivent être reconnus
    preferred_decades = c(1970, 1980, 1990)
  ),
  "surprise" = list(
    ...
    alpha_preference = 0.05, rating_min = 7.6, duration_preference = "any",
    boost_low_votes = TRUE, category = "specifique",
    weights = list(genre = 0.25, quality = 0.50, popularity = 0.05, recency = 0.15, duration = 0.05),
    min_votes = 300,  # Accepter les pépites méconnues
  ...
  "western" = list(
    ...
    genres = c("Western"), alpha_preference = 0.3, rating_min = 7.2, duration_preference = "moyen",
    category = "specifique",
    weights = list(genre = 0.40, quality = 0.45, popularity = 0.10, recency = 0.00, duration = 0.05),
    min_votes = 1000
  )
)
...
apply_emotion_filter <- function(data, emotion_key, custom_alpha = NULL,
  ...
  # Filtrage par votes minimaux (selon l'émotion)
  min_votes_threshold <- if (!is.null(config$min_votes)) config$min_votes else 500
  data <- data %>% filter(votes >= min_votes_threshold)
  
  # Score de correspondance des genres (au lieu d'un filtre strict)
  if (!is.null(config$genres)) {
    data <- data %>%
      rowwise() %>%
      mutate(
        # Proportion de genres requis présents
        genre_match_count = sum(config$genres %in% genres_list),
        genre_match_score = genre_match_count / length(config$genres),
        # Bonus si match parfait
        perfect_genre_match = ifelse(
          all(config$genres %in% genres_list) && 
          length(setdiff(genres_list, config$genres)) == 0, 
          0.15, 
          0
        )
      ) %>%
      ungroup() %>%
      # Garder au moins 50% de correspondance
      filter(genre_match_score >= 0.5)
  } else {
    # Si pas de genres spécifiés (ex: nostalgie)
    data <- data %>% mutate(genre_match_score = 0.5, perfect_genre_match = 0)
  }
  
  # Exclure les genres non désirés (filtre strict)
  ...
  # Score de durée (correspondance avec préférence)
  if (!is.null(custom_duration)) {
    ...
    if (config$duration_preference == "court") {
      data <- data %>% 
        mutate(duration_score = case_when(
          runtime <= 90 ~ 1.0,
          runtime <= 100 ~ 0.8,
          runtime <= 110 ~ 0.5,
          TRUE ~ 0.2
      ...
      data <- data %>% 
        mutate(duration_score = case_when(
          runtime >= 95 & runtime <= 125 ~ 1.0,
          runtime >= 85 & runtime <= 135 ~ 0.8,
          runtime >= 75 & runtime <= 145 ~ 0.5,
          TRUE ~ 0.3
        ))
    ...
    data <- data %>% mutate(duration_score = 0.5)
  }
  ...
  recency_weight <- if (!is.null(config$recency_boost)) config$recency_boost else 0.10
  data <- data %>%
    mutate(
      recency_score = pmax(0, 1 - (current_year - year) / 40)
    )
  
  # Bonus pour décennies préférées (ex: nostalgie)
  if (!is.null(config$preferred_decades)) {
    data <- data %>%
      mutate(
        decade = floor(year / 10) * 10,
        decade_bonus = ifelse(decade %in% config$preferred_decades, 0.15, 0)
      )
  } else {
    data <- data %>% mutate(decade_bonus = 0)
  }
  
  # Score de qualité normalisé (sur l'échantillon filtré)
  data <- data %>%
    mutate(
      quality_score_normalized = (rating - min(rating)) / (max(rating) - min(rating) + 0.001)
    )
  
  # Score de variété (favoriser films moins connus pour "surprise")
  data <- data %>%
    mutate(variety_score = 1 - popularity_score)
  
  # Coefficient de confiance basé sur le nombre de votes
  data <- data %>%
    mutate(
      confidence = pmin(1, log10(votes) / 5),  # Max à 100k votes
      confidence_adjustment = 0.7 + 0.3 * confidence
    )
  
  # Pénalité pour films trop populaires (éviter blockbusters sur-représentés)
  data <- data %>%
    mutate(
      diversity_penalty = ifelse(votes > 500000, 0.08, 
                          ifelse(votes > 300000, 0.04, 0))
    )
  
  # Utiliser les poids multi-critères si disponibles
  if (!is.null(config$weights)) {
    w <- config$weights
    data <- data %>%
      mutate(
        composite_score = 
          w$genre * (genre_match_score + perfect_genre_match) +
          w$quality * quality_score_normalized +
    ...
    data <- data %>%
      mutate(
        composite_score = (1 - alpha_to_use) * quality_score + 
                          alpha_to_use * popularity_score +
                          0.1 * recency_score +
                          decade_bonus -
                          diversity_penalty
      )
  ...
  data <- data %>%
    mutate(composite_score = composite_score * confidence_adjustment)
  
  # Boost pour films peu connus (mode "surprise")
  if (!is.null(config$boost_low_votes) && isTRUE(config$boost_low_votes)) {
    data <- data %>%
      mutate(composite_score = composite_score + variety_score * 0.20)
  }
  
  # Boost de diversité supplémentaire
  if (!is.null(config$diversity_boost)) {
    data <- data %>%
      mutate(composite_score = composite_score + variety_score * config$diversity_boost)
  }
  
  # Trier et retourner
  data %>% 
    arrange(desc(composite_score)) %>%
    # Ajouter un score final normalisé pour affichage
    mutate(final_display_score = pmin(100, composite_score * 100))
}
...
server <- function(input, output, session) {
  ...
  output$results_table <- renderDT({
    data_to_show <- if (input$random > 0 && (is.null(input$emotion_selected) || input$emotion_selected == "")) {
      random_movie() %>%
        mutate(
          composite_score = rating / 10,
          final_display_score = composite_score * 100
        ) %>%
        head(1)
    } else {
    ...
    display <- data_to_show %>%
      transmute(
        ...
        Genres = genres,
        Note = rating,
        Score = round(final_display_score, 1)
      )
